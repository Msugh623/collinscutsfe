          ...
          <div className="container">
            <div className="row align-items-stretch">
              <div className="col-md-8 slideUp" data-aos="fade-up">
                <Player
                  poster={theCreation?.thumbnail}
                  preload="auto"
                  src={theCreation?.source}
                >
                  <ControlBar autoHide={false} className="my-class" />
                </Player>
              </div>
              <div
                className="col-md-4 ml-auto mt-3 mt-sm-0"
                data-aos="fade-up"
                data-aos-delay="100"
              >
                <div className="sticky-content">
                  ...
                  <p className="mb-4 -sm ">
                    <div className="div rounded p-2 shadow">
                      {isEdit == "category" ? (
                        <div>
                          <select
                            className="form-control"
                            autoFocus
                            value={editData.category || theCreation.category}
                            name="category"
                            onChange={handleInput}
                          >
                            {/* CTGS */}
                    ...
                    <div className="div rounded p-2 shadow">
                      {isEdit == "rating" ? (
                        <div>
                          <input
                            ...
                            className="form-control"
                            autoFocus
                            value={editData.rating || theCreation.rating || 0}
                            name="rating"
                            onChange={handleInput}
                          />
                          {/* {editData.rating} */}
                      ...
                      <div className="small">
                        <small className="text-muted">
                          ...
                          Creations are going to be sorted by relevance from
                          heighest to lowest
                        </small>
                      </div>
                    </div>
                  ...
                  <div className="mb-5">
                    <p
                      className="slideUp shadow rounded p-2"
                      onDoubleClick={() =>
                        setIsEdit((prev) =>
                    ...
                    <Link
                      className={`text-dark bg-none text-light growIn me-3 `}
                      onClick={() => {
                        setEditData((prev) => {
                          return {
                            ...prev,
                    ...
                    >
                      {editData?.star ? (
                        <BsStarFill className="fs-3 text-warning" />
                      ) : (
                        <BiStar className="fs-3" />
                      )}
                    </Link>
                    <Link
                      className="readmore custom-navmenu bg-danger text-light growIn"
                      onClick={() => {
                        confirm(
                          "You are about to delete this creation, this irreversible action will tracelessy remove this creation! proceed?"
                        ) &&
                          (async () => {
                            const tst = toast("updating...", {
                              autoClose: false,
                            });
                            try {
